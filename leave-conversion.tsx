...
export default function LeaveConversionScreen() {
  ...
  const [isDarkMode] = useState(false); // Will be connected to global theme later
  ...
  // Calculate total eligible days and monetary value
  const totalEligibleDays = leaveBalances.reduce((sum, leave) => sum + leave.eligible, 0);
  const totalAmountBeforeTax = leaveBalances.reduce((sum, leave) => sum + (leave.eligible * leave.rate), 0);
  const taxAmount = totalAmountBeforeTax * 0.15; // 15% tax for demo purposes
  const netAmount = totalAmountBeforeTax - taxAmount;

  const handleRequestConversion = () => {
    Alert.alert(
      'Leave Conversion Request',
      `You are requesting to convert ${totalEligibleDays} days for a net amount of $${netAmount.toFixed(2)}. This action cannot be undone.`,
      [
        {
          text: 'Cancel',
          style: 'cancel',
    ...
    <View className={`${isDarkMode ? 'bg-gray-900' : 'bg-[#F7F7F7]'} flex-1`}>
      ...
      >
        <View className="flex-row items-center mt-10">
          ...
          <View>
            <Text className="text-white text-xl font-bold">Leave Conversion</Text>
            <Text className="text-blue-100 text-sm mt-1">Convert unused leave days to cash</Text>
          </View>
        </View>
      </LinearGradient>
      ...
      <ScrollView className="flex-1 px-4 mt-6" showsVerticalScrollIndicator={false}>
        ...
        <View className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl p-5 shadow-md mb-6`}>
          ...
          <View className="mb-4">
            ...
            <View className="flex-row justify-between py-3">
              <Text className={`${isDarkMode ? 'text-gray-300' : 'text-gray-600'} font-bold`}>
                Net Amount
              </Text>
        ...
        <View className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl p-5 shadow-md mb-6`}>
          ...
          <View className="mb-2">
            <Text className={`${isDarkMode ? 'text-white' : 'text-[#1A1D23]'} font-medium mb-3`}>
              Your Tax Bracket: 15%
            </Text>
            
            <View className={`${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'} rounded-lg p-4`}>
        ...
        <View className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl p-5 shadow-md mb-6`}>
          ...
          <View className="space-y-3">
            <View className="flex-row items-start">
              <CheckCircle color={isDarkMode ? '#10B981' : '#059669'} size={20} className="mt-0.5" />
              <Text className={`${isDarkMode ? 'text-gray-300' : 'text-gray-600'} ml-3`}>
        ...
        <TouchableOpacity
          className={`rounded-xl p-4 mb-6 ${conversionRequested ? 'bg-green-500' : 'bg-blue-500'} shadow-md`}
